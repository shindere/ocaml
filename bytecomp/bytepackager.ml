  ...
  let rel' =
      ...
      | Reloc_getcompunit cu ->
        ...
        with Not_found ->
          (* PR#5276: unique-ize dotted global names, which appear
             ...
             module. *)
          let Compunit name = cu in
          if String.contains name '.' then
            let new_cu_name =
              Compunit (packagename ^ "." ^ name)
            in
            Reloc_getcompunit new_cu_name
          else
            rel
        end
    | Reloc_setcompunit cu ->
        ...
        with Not_found ->
          ...
          let Compunit name = cu in
          if String.contains name '.' then
            let new_name = packagename ^ "." ^ name in
            Reloc_setcompunit (Compunit new_name)
          else
            rel
        end
  ...
  let kind =
    ...
    if Filename.check_suffix file ".cmi" then
      ...
      Fun.protect ~finally:(fun () -> close_in ic) (fun () ->
        ...
        if compunit.cu_name <> (Compunit name)
        then begin
          let Compunit cu_name = compunit.cu_name in
          raise(Error(Illegal_renaming(name, file, cu_name)))
        end;
        PM_impl compunit)
    end in
  let pm_ident = Compunit name in
  let pm_packed_ident = Compunit (targetname ^ "." ^ name) in
...
let rename_append_pack_member packagename oc state m =
  ...
  | PM_impl compunit ->
      ...
      let subst =
        let id' = ident_of_compunit id in
...
let report_error ppf = function
    Forward_reference(file, (Compunit ident)) ->
      fprintf ppf "Forward reference to %s in file %a" ident
        Location.print_filename file
  | Multiple_definition(file, (Compunit ident)) ->
