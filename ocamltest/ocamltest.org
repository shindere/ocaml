...
It can however be observed that the OCaml compiler is /boostrapped/,
...
Language (DSL) used to annotate the program to test and specify,
in a =(* TEST *)= block at the top of the program to specify how
...
built-in tests and actions and of the test and actions that are specific
...
second line is almost similar but if some modules would have separate
...
ocamltest terminology. An /action/ is anything that can =pass=, =skip= of
...
2. Since we now expect a non-empty output for the compilers, we need to
   ...
   has been created. Of course, the action that check compiler output
   ...
   output but we can double-check that the output is was we expect as a
   reference:
...
may other languages, like the bash shell. Moreover, it is the first
