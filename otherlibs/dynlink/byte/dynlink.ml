...
module Bytecode = struct
  ...
  module Unit_header = struct
    ...
    let name (t : t) =
      let Cmo_format.Compunit cu_name =  t.cu_name in
      cu_name
    let crc _t = None
    ...
    let implementation_imports (t : t) =
      ...
      let required =
        List.filter
          (fun (Cmo_format.Compunit cu) ->
             not (String.contains cu '.'))
          required
      in
      List.map
  ...
  let run lock (ic, file_name, file_digest) ~unit_header ~priv =
    ...
    let clos = with_lock lock (fun () ->
        ...
        with Symtable.Error error ->
          let new_error : DT.linking_error =
            ...
            | Symtable.Undefined_global global ->
              Undefined_global (Symtable.Global.description global)
        ...
        (* PR#5215: identify this code fragment by
           ...
           fragments. *)
        let Cmo_format.Compunit unit_name = compunit.cu_name in
